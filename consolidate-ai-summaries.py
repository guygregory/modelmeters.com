...
<body>
	...
	<script>
		// Language toggle (match main index.html behavior exactly)
		(function initLanguage(){{
			const btn = document.getElementById('btn-language');
			if(!btn) return; // safety
			
			// Check URL params first, then localStorage, default to English
			const urlParams = new URLSearchParams(window.location.search);
			const urlLang = urlParams.get('lang');
			const stored = localStorage.getItem('modelMetersLanguage');
			let currentLang = urlLang || stored || 'en';
			
			// Normalize language codes
			if (currentLang !== 'pl') currentLang = 'en';
			
			// Store the preference if it came from URL
			if (urlLang) localStorage.setItem('modelMetersLanguage', currentLang);
			...
			btn.addEventListener('click', ()=> {{
				...
				localStorage.setItem('modelMetersLanguage', currentLang);
				updateBtn();
			}});
	...
	<script>
		// Back button: navigate to parent, preserving language via ?lang=
		(function(){{
			const btn = document.getElementById('btn-back');
			if (!btn) return;
			btn.addEventListener('click', ()=>{{
				const currentLang = localStorage.getItem('modelMetersLanguage') || 'en';
				window.location.href = '../?lang=' + encodeURIComponent(currentLang);
			}});
		}})();
	</script>
